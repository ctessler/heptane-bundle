...
namespace cfglib {
  ...
  void LoopComputer::computeLoop(Cfg * cfg)	/* public */
  {
	...
	for (std::vector < Node * >::iterator j = predecessors.begin(); j != predecessors.end(); j++)
	  {
		 ...
		 */
		if (dominators.findBB(node) && (node != pred))	// LBesnard  Aug 2016: if (dominators.findBB (node)) replaced by  
		  {
		    isLoopHead = true;
	...
	/* If the current node is the head of a loop */ if (isLoopHead)
	  {
			...
			if (dd.findBB(suc))
			  {
			    ...
			    /* the we add recursively all the others Node */
  ...
  void LoopComputer::computeLoop(Loop * loop, Cfg * cfg, Node * current)	/* private */
  {
    if (!current || !cfg || !loop)
      {
    ...
    /*
       std::vector<Node*> successors = cfg->GetSuccessors (current);
       std::vector<Node*>::iterator successor;
       for (successor  = successors.begin();
       successor != successors.end();
       successor++)
       {
       if (*successor) {
       dbg_loop (std::cerr << "new successor" << std::endl;);
       computeLoop (loop, cfg, *successor);
       }
       }
     */
