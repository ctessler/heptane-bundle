...
// ---------------------------------------
IPETAnalysis::IPETAnalysis(Program * p, int m, int used_solver, bool generate_wcet_info, bool generate_node_freq, int nb_icache_levels, int nb_dcache_levels, 
			   map < int, int >CacheLatency, int latPerfectIcache, int latPerfectDcache):Analysis(p)
{
  ...
  PerfectICacheLatency = latPerfectIcache;
  PerfectDCacheLatency = latPerfectDcache;
  ...
  if (NbICacheLevels != 0 )
    { 
      if ( /* m == METHOD_NOPIPELINE_PERFECTICACHE_DCACHE ||*/  m == METHOD_NOPIPELINE_PERFECTICACHE_PERFECTDCACHE )
	Logger::addFatal("IPETAnalysis: incorrect value of  nbcache levels for IPET analysis with no cache");
    }
  else
    {
      if ( ( m != METHOD_NOPIPELINE_PERFECTICACHE_PERFECTDCACHE) /* && ( m != METHOD_NOPIPELINE_PERFECTICACHE_DCACHE)*/ )
	Logger::addFatal("IPETAnalysis: incorrect value of nbcache levels for IPET analysis with no cache");
    }
  ...
  for (int l = 1; l <= NbICacheLevels; l++)
    {
      ...
      levelAccessCost[l] = CacheLatency[l];
    }
			  ...
			  for (int currentCacheLevel = 1; currentCacheLevel <= NbICacheLevels; currentCacheLevel++)
			    {
			      for (unsigned int ic = 0; ic < nc; ic++)
				{
				  string attributeName = AnalysisHelper::mkContextAttrName(CodeCHMC[currentCacheLevel], contexts[ic]->getStringId());
				  if (! this->CheckCacheAttributes(vi[inst], attributeName))  // currently return always true!
				    return false;
				}
			    }
			}
...
{
  int currentAccessCost = levelAccessCost[numCache];
  ...
  if (classif != "AU")  
    {
      ...
      DataCacheLevel_latency( levelAccessCost[numCache], true, wcet_first, wcet_next, *never_accessed_data, *always_accessed_data, *occurrence_bound_data);
...
void IPETAnalysis::ComputeInstrExecutionTime_NOPIPELINE_ICACHE_DCACHE(Instruction *vinstr, Context * context,  unsigned int frequency,  int *wcet_first, int *wcet_next )
{
  int currentCache;
  string vinstr_code;
  bool countFirst, countNext, always_accessed_data,  never_accessed_data, isLoad;
  unsigned int occurrence_bound_data;

  ComputeInstrExecutionTime_NOPIPELINE_STORE_DCACHE(vinstr, wcet_first, wcet_next);
  isLoad = Arch::isLoad(vinstr->GetCode());
...
bool IPETAnalysis::PerformAnalysis()
{
  ...
  solver->generate_declarations(stde, vid);
  ...
  string tmpFileName = mktemp(fileNameTemplate);

  // Compiler: the use of `mktemp' is dangerous, better use `mkstemp'
  // LBesnard. error ( for cplex, ok for lp_solve ) with: mkstemp(fileNameTemplate);  string tmpFileName = fileNameTemplate;
