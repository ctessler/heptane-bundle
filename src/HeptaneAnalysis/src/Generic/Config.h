...
public:
  ...
  void ExecuteFromXml (string xml_file);
  ...
  // Latency for cache access 
  // NB: cache levels are number from 1 (L1 cache) to N (last level cache)
  // NB: all caches at the same level (dcache, icache) have the same latency
  int getCacheLatency (int level);
  ...
  const map < int, int >&getCacheLatencies ();
