...
Config::Config ()
{
  ...
  for (unsigned int i = 1; i < NB_MAX_CACHE_LEVEL + 1; i++)
    cache_latencies[i] = 0;
...
Analysis* Config::mkAnalyzerObject(string directive, Program *p, ParamAnalysis *pa)
{
  ...
  if (directive == "CODELINE") { return new CodeLine (p, ((ParamCodeLine *) pa)->binary_file, arch_name, ((ParamCodeLine *) pa)->binary_addr2line); }
  if (directive == "HTMLPRINT") { return new HtmlPrint (p, ((ParamHtmlPrint *) pa)->html_file, ((ParamHtmlPrint *) pa)->colorize); }
  if (directive == "CACHESTATISTICS") { return new CacheStatistics (p, GetCaches ()); }
  ...
  if (directive == "ICACHE")
    {
      ...
      return new ICacheAnalysis (p, cp->nbsets, cp->nbways, cp->cachelinesize, cp->replacement_policy, ps->level, ps->apply_must, ps->apply_persistence, ps->apply_may, ps->keep_age);
    }
  ...
  if (directive == "DCACHE")
    {
      ...
      assert (cp != NULL);
      if (ps->level > MaxLevelCacheAnalysis) MaxLevelCacheAnalysis=ps->level;
      return new DCacheAnalysis (p, cp->nbsets, cp->nbways, cp->cachelinesize, cp->replacement_policy, ps->level, ps->apply_must, ps->apply_persistence, ps->apply_may);
    }
  if (directive == "PIPELINE")
    {
      int nbicache = getNbICacheLevels ();
      int nbdcache = getNbDCacheLevels ();
      assert (nbicache == nbdcache);
  ...
  if (directive == "IPET")
    {
      ...
      return new IPETAnalysis (p, ps->m, ps->solver, ps->attach_WCET_info, ps->generate_node_freq, getNbICacheLevels (), getNbDCacheLevels (), cache_latencies, ps->latICache, ps->latDCache);
    }
...
void
Config::ExecuteFromXml (string xml_file)
{
  XmlDocument xmldoc (xml_file);
  ListXmlTag lt;
  bool b;
  ...
  for (unsigned int i = 0; i < ltanalysis.size (); i++)
      ...
      if ( analysis_name != "ENTRYPOINT")
	{
	  ...
	  Logger::clean ();
	  bool res = a->CheckPerformCleanup ();
...
// ---------------------------------------------------
int
Config::getCacheLatency (int level)
{
  // WHY ?? we have always a CACHE ?
  // assert (level >= 1 && (level <= nb_icache_levels || level <= nb_dcache_levels));
  return cache_latencies[level];
}
...
CacheParam::CacheParam (XmlTag const &tag)
{
  ...
  assert (stype == "icache" || stype == "dcache");
  if (stype == "icache")
    this->type = ICACHE;
  else
    this->type = DCACHE;
  ...
  ParamAnalysis (tag)
{
  ...
  assert (s == "true" || s == "false" || s == "");
  this->printcallgraph = (s == "true");

  s = tag.getAttributeString ("printloopnest");
  assert (s == "true" || s == "false" || s == "");
  this->printloopnest = (s == "true");

  s = tag.getAttributeString ("printcfg");
  assert (s == "true" || s == "false" || s == "");
  this->printcfg = (s == "true");

  s = tag.getAttributeString ("printWCETinfo");
  assert (s == "true" || s == "false" || s == "");
  this->printWCETinfo = (s == "true");
}
...
{
  string s;
  bool pipeline, icache, dcache;

  s = tag.getAttributeString ("attach_WCET_info");
  assert (s == "true" || s == "false" || s == "");
  this->attach_WCET_info = (s == "true");

  s = tag.getAttributeString ("generate_node_freq");
  assert (s == "true" || s == "false" || s == "");
  this->generate_node_freq = (s == "true");
  ...
  ListXmlTag lt = tag.searchChildren ("METHOD");
  assert(lt.size () == 1);
  XmlTag method = lt[0];

  s = method.getAttributeString ("PIPELINE");
  assert (s == "true" || s == "false");
  pipeline = ( s == "true" );

  s = method.getAttributeString ("ICACHE");
  assert (s == "true" || s == "false");
  icache = ( s == "true" );

  s = method.getAttributeString ("DCACHE");
  assert (s == "true" || s == "false");
  dcache = ( s == "true" );

  this->latICache = 1;
  this->latDCache = 0;
  if (icache) this->latICache=method.getAttributeInt("latICache");
  if (dcache) this->latDCache=method.getAttributeInt("latDCache");
  ...
  if (pipeline)
      ...
      if (icache)
	{
	  ...
	  if (dcache)
	    this->m = METHOD_PIPELINE_ICACHE_DCACHE;
	  else
	    this->m = METHOD_PIPELINE_ICACHE_PERFECTDCACHE;
	}
    else
      {
	Logger::addFatal ("Config: IPET error configuration: PIPELINE, PERFECTICACHE not implemented");
      }
    }
  else
    {
      ...
      {
	if (dcache)
	  this->m = METHOD_NOPIPELINE_ICACHE_DCACHE;
	else
	  this->m = METHOD_NOPIPELINE_ICACHE_PERFECTDCACHE;
      }
    else
      {
	if (dcache)
	  this->m = METHOD_NOPIPELINE_PERFECTICACHE_DCACHE;
	else
	  this->m = METHOD_NOPIPELINE_PERFECTICACHE_PERFECTDCACHE;
      }
    }
}
