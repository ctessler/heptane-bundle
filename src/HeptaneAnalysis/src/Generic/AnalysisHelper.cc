...
bool AnalysisHelper::applyToAllNodesRecursive(Program * p, t_node_function f, void *param)
{
  ...
  while (vn.size() != 0)
      ...
      if (n->IsCall())
	{
	  ...
	  assert(callee != NULL);
	  if (callee->IsExternal() == false)
	    {
	      Node *entry = callee->GetStartNode();
	      if (entry->HasAttribute(VisitedInternalAttributeName) == false)
		{
  ...
  for (unsigned int c = 0; c < lc.size(); c++)
    {
      ...
      // Check loop bounds
      if (!cg.isDeadCode(lc[c]))	// modif LBesnard 26 May 2016, before :(cg.isDeadCode (lc[c])) 
	{
