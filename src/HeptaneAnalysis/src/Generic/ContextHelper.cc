...
std::vector < ContextualNode > GetContextualPredecessors (const ContextualNode & position)
{
  ...
  if (function->GetStartNode () == position.node && position.context->getCallerContext () != NULL)
    {
      contextual_predecessors.push_back (ContextualNode (position.context->getCallerContext (), position.context->getCallerNode ()));
    }
