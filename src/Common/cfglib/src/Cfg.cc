...
namespace cfglib {
  ...
  Cfg* Cfg::Clone(CloneHandle& handle) {
    ...
    //loops
    for(std::vector<Loop*>::iterator it = loops.begin();
	it != loops.end();
	it++){
    ...
    for(std::vector<Node*>::iterator it = curCFGendNodes.begin();
	it != curCFGendNodes.end();
	it++){
  ...
  std::vector<Node*> Cfg::GetCallNodes(void) {
    ...
    for(std::vector<Node*>::iterator it = nodes.begin(); it != nodes.end(); it++)
      {
	if((*it)->IsCall()) { callNodes.push_back(*it); }
      }
    return callNodes;
  }
  ...
  void Cfg::RemoveEdge(Edge *e) {
    for (std::vector<Edge*>::iterator it = this->edges.begin() ; it
	   != this->edges.end() ; it++) {
      if ((*it) == e) {
	delete e;
	this->edges.erase(it);
  ...
  Edge* Cfg::FindEdge(Node const* origin, Node const* destination) const {
    for (std::vector<Edge*>::const_iterator it = this->edges.begin() ; it
	   != this->edges.end() ; it++) {
  ...
  std::vector<Node*> Cfg::GetSuccessors(Node* node) {
    ...
    for (std::vector<Edge*>::const_iterator it = this->edges.begin() ; 
	 it != this->edges.end() ; 
	 it++) {
      if ((*it)->origin == node) {
  ...
  void Cfg::ReadXml(XmlTag const* tag, Handle& hand) {
    ...
    std::vector<std::string> endnodes_vector(cfglib::helper::split_string(
									  attr_endnodes, ", "));
    this->endNodes.resize(endnodes_vector.size());
    for (unsigned int i=0; i<endnodes_vector.size(); i++) {
      this->endNodes[i]=NULL;
      hand.addID_handle(endnodes_vector[i],
			(Serialisable **)&(this->endNodes[i]));
    }
	...
	std::cerr << "XML parsing WARNING : unrecognised child of <CFG>"
