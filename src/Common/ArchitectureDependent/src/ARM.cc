...
bool ARM::isPcInInputResources(const ObjdumpInstruction & instr)
{
  ...
  // Checking if "pc" is in the vector of inputs
  if (find(inputs.begin(), inputs.end(), "pc") == inputs.end())
    {
      return false;
    }

  // Checking if "pc" is not in the outputs
...
vector < ObjdumpWord > ARM::getWordsFromInstr(const ObjdumpInstruction & instr1, const ObjdumpInstruction & instr2, vector < ObjdumpWord > words,
					      bool & is_instr2_consumed)
{
  ...
  if (getInstructionTypeFromMnemonic(instr1.mnemonic)->isLoad() && getInstructionTypeFromMnemonic(instr1.mnemonic)->isPredicated() == false)
    {
...
/**
   ...
   @param TypeOperand: is the offset type value, element of { immediate_offset, zero_offset, register_offset, scaled_register_offset}
